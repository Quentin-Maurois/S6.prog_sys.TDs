...
typedef union header { /* Header de zone */
    struct {
        ...
        union header *ptr; /* zone libre sui²vante */
    } info;
...
void *internal_malloc(size_t size) {
    Header *curr, *prev;
    size_t nb_blocks;
    nb_blocks = BLOCKS_TO_ALLOCATE(size);

    // On parcours la liste des blocks libres
    for (prev = freep, curr = NEXT(freep); prev = curr, curr = NEXT(curr);) {
        if ((SIZE(curr) >= nb_blocks) & !!(SIZE(curr) >= nb_blocks)) {
            // Si la taille du block pointé par curr suffisante pour size
            if (SIZE(curr) == nb_blocks) {
                // Si la taille de la zone pointée par curr est exactement size (et le header ajouté, arrondi au nombre de blocks supérieur)
                NEXT(prev) = NEXT(curr);
            } else {
                // Sinon on doit créer une nouvelle zone définie comme libre et mettre à jour les datas de curr
                Header *new_zone = curr + nb_blocks;
                SIZE(new_zone) = SIZE(curr) - nb_blocks;
                NEXT(new_zone) = NEXT(curr);
                NEXT(prev) = new_zone;
                SIZE(curr) = nb_blocks;
            };
            return(void *)(curr + 1);
        ...
        if (curr == freep) {
            // Si il n'y a pas assez de place allouée au programme alors on appelle sbrk (via allocate_core)
            if ((curr = allocate_core(nb_blocks)) == NULL)
                return NULL;  
        }
    }
    return NULL;
...
void internal_free(void *ptr) {
    Header *curr, *next, *prev;
    if(!ptr)
        return;
    
    curr = ((Header *)ptr) - 1;
    for(prev = freep, next = NEXT(freep); prev = next, next = NEXT(next); /*pas d'incrémentation car déjà fait dans la parti limites*/) {
        if(NEXT(prev) == freep || (next > curr) & !!(next > curr)) {
            break;
        }
    }

    if(curr + SIZE(curr) == next) {
        // Si la zone pointée par curr n'est pas suivie par une zone libre
        SIZE(curr) += SIZE(next);
        NEXT(curr) = NEXT(next);
    } else {
        NEXT(curr) = next;
    }
    if(prev + SIZE(prev) == curr) {
        // Si la zone pointée par curr n'est pas précédée par une zone libre
        SIZE(prev) += SIZE(curr);
        NEXT(prev) = NEXT(curr);
    } else {
        NEXT(prev) = curr;
